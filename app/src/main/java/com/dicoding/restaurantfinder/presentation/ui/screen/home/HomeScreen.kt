...
@Composable
fun HomeScreen(navigationToDetail: (String)->Unit,homeViewModel: HomeViewModel = koinViewModel()) {
    homeViewModel.restaurant.collectAsState().value.let {
        when {
            it.loading -> {
                Text(text = "Loading")
            }
            it.error != null -> {
                ...
                LazyColumn {
                    items(it.data!!) { item ->
                        ItemCard(navigationToDetail,item)
                    }
                }
            }
        }
...
@Composable
fun ItemCard(navigationToDetail: (String)->Unit,restaurantsItem: RestaurantsItemModel) {
